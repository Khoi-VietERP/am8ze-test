...
class IrActionsReport(models.Model):
    ...
    def export_doc_by_template(self, file_template_data=None, suffix='docx', file_name_export='export1', datas={}):
        if self != self.env.ref('create_ms_word_report.ms_word_report_statement'):
            res = super(IrActionsReport, self).export_doc_by_template(file_template_data=file_template_data, suffix=suffix, file_name_export=file_name_export, datas=datas)
            return res
        else:
            simple_merge = {}
            populating_tables = {}
            ...
            context_data = {}
            try:
                context_data = self.env['report.'+self.report_name].browse()._get_report_values(datas.ids)
            except:
                pass
            ...
            for field in fields:
                ...
                if childs[0] == 'datalines':
                    check = True
                if field in context_data or check:
                    if not check and field in context_data:
                        value = context_data.get(field, False)
