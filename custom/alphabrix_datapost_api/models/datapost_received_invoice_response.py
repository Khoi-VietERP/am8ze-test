...
class DatapostReceivedOrder(models.Model):
    ...
    def action_parse_document(self):
        ...
        sender_id = self.env['res.partner'].search([('l10n_sg_unique_entity_number', 'in', name_peppol)], limit=1)
        ...
        document_reference_xml = self.xml_content.split('<cac:DocumentReference>')[1].split('</cac:DocumentReference>')[
            0]
        ...
        if move:
            ...
            for line_xml in lines_xml:
                if line_xml.find('</cac:DocumentResponse>') != -1:
                    ...
                    effective_date = response_xml.split('<cbc:EffectiveDate>')[1].split('</cbc:EffectiveDate>')[
                        0] if len(response_xml.split('<cbc:EffectiveDate>')) > 1 else None
                    issue_date = response_xml.split('<cbc:IssueDate>')[1].split('</cbc:IssueDate>')[0] if len(
                        response_xml.split('<cbc:IssueDate>')) > 1 else None
                    response_reason = response_xml.split('<cbc:StatusReason>')[1].split('</cbc:StatusReason>')[
                        0] if len(response_xml.split('<cbc:StatusReason>')) > 1 else ''
                    response_reason_code = \
                    response_xml.split('<cbc:StatusReasonCode')[1].split('</cbc:StatusReasonCode>')[0].split('>')[
                        1] if len(response_xml.split('<cbc:StatusReasonCode')) > 1 else ''
