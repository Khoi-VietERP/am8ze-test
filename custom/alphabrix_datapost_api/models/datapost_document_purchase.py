...
<Order xmlns="urn:oasis:names:specification:ubl:schema:xsd:Order-2"
    ...
    def _generate_document_buyer(self):
        ...
        if not company_id.l10n_sg_unique_entity_number:
            raise UserError(_("Need setup 'UEN' for company."))
        company_peppol_name, company_peppol_company_name = company_id.get_peppol_db_company()
        peppol_name, peppol_company_name = company_partner_id.get_peppol_db_partner()
        if not company_peppol_name:
            raise UserError(_("No 'Peppol ID' for company."))
        company_peppol_id = company_peppol_name
        peppol_prefix = company_peppol_id.split(':')[0]
        peppol_id = company_peppol_id.split(':')[1]
        ...
        result = f'''<cac:BuyerCustomerParty>
    		<cac:Party>
    			...
    			<cac:PartyLegalEntity>
    				<cbc:RegistrationName>{peppol_company_name}</cbc:RegistrationName>
    ...
    def _generate_document_seller(self):
        if not self.purchase_id.partner_id.l10n_sg_unique_entity_number:
            raise UserError(_("Need setup 'UEN' for this customer."))

        if not self.purchase_id.partner_id.email:
            raise UserError(_("Need setup 'Email' for this customer."))
        peppol_name, peppol_company_name = self.purchase_id.partner_id.get_peppol_db_partner()
        peppol_id = peppol_name or '0195:SGTST199404610D'
