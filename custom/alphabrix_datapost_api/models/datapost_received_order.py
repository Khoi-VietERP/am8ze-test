...
class DatapostReceivedOrder(models.Model):
    ...
    def action_parse_document(self):
        ...
        peppol_buyer_reference = self.xml_content.split('<cbc:CustomerReference>')[1].split('</cbc:CustomerReference>')[
            0]
        ...
        customer_id = self.env['res.partner'].search([('l10n_sg_unique_entity_number', 'in', name_peppol)], limit=1)
        if not customer_id:
