...
class BalanceSheetReportXlsx(models.AbstractModel):
    _name = 'report.alphabricks_bs_report.balance_sheet_report_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        self.record = record
        self.sheet = workbook.add_worksheet('BS Report')

        self.sheet.write_string(0, 0, data['company_name'], self.format_title)
        self.sheet.write_string(1, 0, 'Balance Sheet Report', self.format_title)
        self.sheet.write_string(2, 0, 'As of %s'% (data['end_date']) , self.format_header)
        ...
        if not data['check_cmp']:
            ...
            self.sheet.write_string(self.row_pos, 1, 'Balance', self.format_header)
            ...
            for line in data['lines_data']:
                ...
                if line['type'] == 'line':
                    ...
                    for balance_cmp in data['balance_cmp_list']:
                        count += 1
                        self.sheet.write_string(self.row_pos, count, space + line[balance_cmp], self.line_body_right)
                else:
                    self.sheet.write_string(self.row_pos, 0, space + line['name'], self.line_body_bold)
                    count = 0
                    for balance_cmp in data['balance_cmp_list']:
                        count += 1
                        self.sheet.write_string(self.row_pos, count, space + line[balance_cmp], self.line_body_bold_right)