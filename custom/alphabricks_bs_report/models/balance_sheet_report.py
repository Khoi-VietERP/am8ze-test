...
class balance_sheet_report(models.TransientModel):
    ...
    def _compute_report_balance(self, account_ids, type = False):
        res = {}
        fields = ['balance']
    ...
    def get_report_datas(self):
        ...
        if self.check_cmp:
            ...
            data = self.with_context({'date_from': False, 'date_to': self.end_date})._compute_report_balance(account_ids)
            data = self._compute_data(data, 1)
            fields = ['balance']
            new_data = []
            for d in data:
