...
class AccountMove(models.Model):
    ...
    def _get_report_values_barcode(self):
        self.ensure_one()
        pick_data = []
        attention = self.env['res.partner'].search([('parent_id', '=', self.partner_id.id), ('type', '=', 'contact')],
                                                   limit=1).name
        bill_attn = self.env['res.partner'].search([('parent_id', '=', self.partner_id.id), ('type', '=', 'contact')],
                                                   limit=1).name
        ship_to_attn = self.env['res.partner'].search([('parent_id', '=', self.partner_shipping_id.id), ('type', '=', 'delivery')],
                                                   limit=1).name
        ...
        data = {
            ...
            'bill_to': self.partner_id.name,
            'bill_street': self.partner_id.street if self.partner_id.street else '',
            'bill_street2': self.partner_id.street2 if self.partner_id.street2 else '',
            'bill_city': self.partner_id.city if self.partner_id.city else '' ,
            'bill_zip': self.partner_id.zip if self.partner_id.zip else '' ,
            'bill_tel': self.partner_id.phone or self.partner_id.mobile or '',
            'bill_fax': '',
            'bill_attn': bill_attn,
            'attention': attention,
            'shipping_to': self.partner_shipping_id.name,
            'shipping_to_street': self.partner_shipping_id.street if self.partner_shipping_id.street else '',
            'shipping_to_street2': self.partner_shipping_id.street2 if self.partner_shipping_id.street2 else '',
            'shipping_to_city': self.partner_shipping_id.city if self.partner_shipping_id.city else '',
            'shipping_to_zip': self.partner_shipping_id.zip if self.partner_shipping_id.zip else '',
            'ship_to_tel': self.partner_shipping_id.phone or self.partner_shipping_id.mobile or '',
            'ship_to_fax': '',
            'ship_to_attn': ship_to_attn,
    ...
    def get_do_name(self):
        sale_id = self.get_sale_order()
        picking_id = self.env['stock.picking']
        if sale_id:
            picking_ids = sale_id.picking_ids.filtered(lambda p: p.picking_type_code == 'outgoing')
            if picking_ids:
                picking_id = picking_ids[0]
        return picking_id.name or ''

    def get_sale_order(self):
        sale_id = self.env['sale.order'].search(
            [('id', 'in', self.invoice_line_ids.mapped('sale_line_ids').mapped('order_id').ids)], limit=1)
        return sale_id