...
WebClient.include({
    ...
    refresh: function(message) {
    	...
    	if (!this.call('bus_service', 'isMasterTab') || session.uid !== message.uid && 
			...
			var action_env_ids = []
			if (action.env && action.env.ids) {
				action_env_ids = action.env.ids
			}
    		if(controller.widget.isMultiRecord && (message.create || _.intersection(message.ids, action_env_ids) >= 1)) {
