...
class DbBackup(models.Model):
    ...
    def _get_base_url(self, is_port=True):
            ...
            if len(domain) > 1:
                if ":" not in domain:
                    ...
                    try:
                        ...
                        port_index = -(len(domain) - int(index) - 1)
                        port = domain[port_index:]
                        url = domain[:int(index)]
        return port if is_port else url

    @api.model
    def _create_default_db_backup(self):
        data_default = self.default_get(self._fields.keys())
        data_default.update({
            'host': self._get_base_url(False),
            'port': self._get_base_url(),
            'folder': '/opt/odoo/addons/backups',
            'autoremove': True,
            'days_to_keep': 5,
            'sftp_write': True,
            'sftp_host' : 'bk1.am8ze.com',
            'sftp_user' : 'root',
            'sftp_password' : '/var/lib/odoo/.ssh/id_rsa',
            'sftp_path' : '/root/odoo/backups',
        })
        self.create(data_default)
    ...
    def test_sftp_connection(self, context=None):
        ...
        for rec in self:
            ...
            try:
                ...
                s.load_system_host_keys(password_login)
                s.connect(ip_host, username=username_login, timeout=10)
                sftp = s.open_sftp()
