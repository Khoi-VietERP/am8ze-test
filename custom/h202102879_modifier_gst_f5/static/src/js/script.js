odoo.define('h202102879_modifier_gst_f5.GstF5Main', function (require) {
    'use strict';
    ...
    var _t = core._t;
    var exports = {};
    var GstF5Main = AbstractAction.extend({
        template: 'GstF5Main',
        events: {
        ...
        view_move_line_form: function (event) {
            ...
            self._rpc({
                model: 'account.gstf5.trans',
                method: 'get_model_open',
                args: [[self.wizard_id], $(event.currentTarget).data('move-id')],
            }).then(function (url) {
        ...
        plot_data: function (initial_render = true) {
            ...
            self._rpc({
                ...
                if (initial_render) {
                    self.$('.py-control-panel').html(QWeb.render('FilterGstF5', {get_info: datas,}));
                    self.$el.find('#date_from').datepicker({dateFormat: 'dd-mm-yy'});
                    self.$el.find('#date_to').datepicker({dateFormat: 'dd-mm-yy'});
                }
                self.$('.py-data-container-orig').html(QWeb.render('GstF5MainList', {
                    get_info: datas,
                }));
        ...
        print_pdf: function (e) {
            ...
            self._rpc({
                ...
                action['context'] = {
                    'active_model': 'account.gstf5.trans',
    ...
    core.action_registry.add('gst.f5', GstF5Main);
