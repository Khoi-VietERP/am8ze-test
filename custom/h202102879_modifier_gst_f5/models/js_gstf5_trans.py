...
class js_gstf5_trans(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        self.sheet.merge_range(0,4,0,7,report_data['name'], self.format_tilte)
        ...
        header_list = ['','Date','Account','Transaction Type','No.','Memo/Description','Name','GST Code','GST Rate','Net Amount','Balance']
        col = 0
        row = 4
        ...
        self.sheet.merge_range(row,1,row,10,'')
        ...
        for box_line1 in report_data['detail_box1']:
            ...
            self.sheet.write_string(row, 10, str(box_line1['balance']),self.line_right)
        ...
        self.sheet.write_string(row, 9, str(report_data['box1_tot_net_amount']), self.total_right)
        self.sheet.write_string(row, 10, '', self.total_right)
        self.sheet.write_string(row, 11, '', self.total_right)

        # TODO box2
        row += 1
        self.sheet.write_string(row, 0, 'Box 2 Total value of zero-rated supplies (excluding GST)',
                                self.line_header)
        self.sheet.merge_range(row, 1, row, 10, '')

        for box_line2 in report_data['detail_box2']:
            row += 1
            self.sheet.write_string(row, 1, box_line2['date'].strftime('%d-%m-%Y'), self.line)
            self.sheet.write_string(row, 2, box_line2['account'], self.line)
            self.sheet.write_string(row, 3, box_line2['move_type'], self.line)
            self.sheet.write_string(row, 4, box_line2['move_name'], self.line)
            self.sheet.write_string(row, 5, box_line2['move_note'], self.line)
            self.sheet.write_string(row, 6, box_line2['move_partner'] or '', self.line)
            self.sheet.write_string(row, 7, box_line2['gst_code'], self.line)
            self.sheet.write_string(row, 8, box_line2['gst_rate'], self.line)
            self.sheet.write_string(row, 9, box_line2['net_amount'], self.line_right)
            self.sheet.write_string(row, 10, str(box_line2['balance']), self.line_right)

        row += 1
        self.sheet.write_string(row, 0, 'Total for Box 2 Total value of zero-rated supplies (excluding GST)',
                                self.line_header)
        self.sheet.merge_range(row, 1, row, 8, '')
        self.sheet.write_string(row, 9, str(report_data['box2_tot_net_amount']), self.total_right)
        self.sheet.write_string(row, 10, '', self.total_right)
        self.sheet.write_string(row, 11, '', self.total_right)

        # TODO box3
        row += 1
        self.sheet.write_string(row, 0, 'Box 3 Total value of exempt supplies (excluding GST)',
                                self.line_header)
        self.sheet.merge_range(row, 1, row, 10, '')

        for box_line3 in report_data['detail_box3']:
            row += 1
            self.sheet.write_string(row, 1, box_line3['date'].strftime('%d-%m-%Y'), self.line)
            self.sheet.write_string(row, 2, box_line3['account'], self.line)
            self.sheet.write_string(row, 3, box_line3['move_type'], self.line)
            self.sheet.write_string(row, 4, box_line3['move_name'], self.line)
            self.sheet.write_string(row, 5, box_line3['move_note'], self.line)
            self.sheet.write_string(row, 6, box_line3['move_partner'] or '', self.line)
            self.sheet.write_string(row, 7, box_line3['gst_code'], self.line)
            self.sheet.write_string(row, 8, box_line3['gst_rate'], self.line)
            self.sheet.write_string(row, 9, box_line3['net_amount'], self.line_right)
            self.sheet.write_string(row, 10, str(box_line3['balance']), self.line_right)

        row += 1
        self.sheet.write_string(row, 0, 'Total for Box 3 Total value of exempt supplies (excluding GST)',
                                self.line_header)
        self.sheet.merge_range(row, 1, row, 8, '')
        self.sheet.write_string(row, 9, str(report_data['box3_tot_net_amount']), self.total_right)
        self.sheet.write_string(row, 10, '', self.total_right)
        self.sheet.write_string(row, 11, '', self.total_right)
        ...
        for box_line5 in report_data['detail_box5']:
            ...
            self.sheet.write_string(row, 10, str(box_line5['balance']), self.line_right)

        row += 1
        self.sheet.write_string(row, 0, 'Total for Box 5 Total value of taxable purchases (excluding GST)',
                                self.line_header)
        self.sheet.merge_range(row, 1, row, 8, '')
        self.sheet.write_string(row, 9, str(report_data['box5_tot_net_amount']), self.total_right)
        self.sheet.write_string(row, 10, '', self.total_right)
        self.sheet.write_string(row, 11, '', self.total_right)
        ...
        for box_line6 in report_data['detail_box6']:
            ...
            self.sheet.write_string(row, 10, str(box_line6['balance']), self.line_right)

        row += 1
        self.sheet.write_string(row, 0, 'Total for Box 6 Output tax due',
                                self.line_header)
        self.sheet.merge_range(row, 1, row, 8, '')
        self.sheet.write_string(row, 9, str(report_data['box6_tot_net_amount']), self.total_right)
        self.sheet.write_string(row, 10, '', self.total_right)
        self.sheet.write_string(row, 11, '', self.total_right)
        ...
        for box_line7 in report_data['detail_box7']:
            ...
            self.sheet.write_string(row, 10, str(box_line7['balance']), self.line_right)

        row += 1
        self.sheet.write_string(row, 0, 'Total for Box 7 Input tax and refunds claimed',
                                self.line_header)
        self.sheet.merge_range(row, 1, row, 8, '')
        self.sheet.write_string(row, 9, str(report_data['box7_tot_net_amount']), self.total_right)
        self.sheet.write_string(row, 10, '', self.total_right)
        self.sheet.write_string(row, 11, '', self.total_right)