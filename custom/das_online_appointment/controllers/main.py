...
class OnlineAppointment(http.Controller):
    ...
    def select_appointees(self, criteria='default', appointment_option=False):
        ...
        if appointment_option.user_specific:
            ...
            slots = request.env['das.time.slot'].sudo().search([('user_id', 'in', user_allowed_ids)])
            appointee_ids = [s.user_id.id for s in slots]
        else:
            slots = request.env['das.time.slot'].sudo().search([])
            appointee_ids = [s.user_id.id for s in slots]
    ...
    def prepare_values(self, form_data=False, default_appointee_id=False, criteria='default'):
        ...
        carrier_types = request.env['das.carrier.type'].sudo().search([])
        warehouses = request.env['stock.warehouse'].sudo().search([])
        time_slots = request.env['das.time.slot'].sudo().search([])
        pallet_type_ids = request.env['pallet.type'].sudo().search([])
        purchase_ids = request.env['purchase.order'].sudo().search([('partner_id', '=', request.env.user.partner_id.id)])
        ...
        values = {
            ...
            'pallet_type_ids' : pallet_type_ids,
        ...
        if form_data:
            ...
            if appointee_id and appointment_option_id and appointment_date:
                ...
                days_with_free_slots = self.get_days_with_free_slots(timeslot_id,
    ...
    def online_appointment(self, **kw):
        ...
        if request.env.user._is_public():
            param = request.env['ir.config_parameter'].sudo().search([('key', '=', 'das_online_appointment')], limit=1)
            if not param or param.value.lower() != 'public':
                return request.render('das_online_appointment.only_registered_users')
        values = self.prepare_values(default_appointee_id=kw.get('appointee', False))

        return request.render('das_online_appointment.make_appointment', values)
    ...
    def online_appointment_confirm(self, **post):
        ...
        try:
            slot = request.env['das.time.slot'].sudo().search([('id', '=', int(post.get('timeslot_id', 0)))])
        except:
            slot = False
        if not slot:
        ...
        try:
            date_start = datetime.datetime.strptime(post['schedule_date'], '%m/%d/%Y').strftime('%Y-%m-%d')
            day_slot = date_start + ' ' + functions.float_to_time(slot.slot)
            start_datetime = self.ld_to_utc(day_slot, request.env.user.id)
        except:
            error['schedule_date'] = True
        ...
        try:
            carrier_type_id = int(post.get('carrier_type_id', 0))
        except:
            carrier_type_id = False

        try:
            warehouse_id = int(post.get('warehouse_id', 0))
        except:
            warehouse_id = False

        try:
            purchase_id = int(post.get('purchase_id', 0))
        except:
            purchase_id = False

        try:
            pallet_type_id = int(post.get('pallet_type_id', 0))
        except:
        ...
        appointment = request.env['calendar.event'].sudo().with_context(detaching=True).create({
            ...
            'stop': (start_datetime + datetime.timedelta(minutes=round(slot.slot * 60))).strftime("%Y-%m-%d %H:%M:%S"),
            'duration': slot.slot,
            'carrier_id': request.env.user.partner_id.id,
            'carrier_type_id': carrier_type_id,
            'warehouse_id': warehouse_id,
            'po_number': purchase_id,
            'direction': post.get('direction', False),
            'pallet_type_id': pallet_type_id,
            'cases': post.get('cases', ''),
            'carrier_number': post.get('carrier_number', ''),
            'pallets': post.get('pallets', ''),
    ...
    def confirmed(self, **post):
        ...
        if not appointment:
            ...
            return request.render('das_online_appointment.thanks', values)
    ...
    def check_slot_is_possible(self, option_id, appointment_date, appointee_id, slot_id):
        ...
        if not appointee_id:
            appointee_id = request.env.user.id
        ...
        if not option:
            return False
        slot = request.env['das.time.slot'].sudo().search([('id', '=', slot_id)])
        if not slot:
            return False
    ...
    def get_free_appointment_slots_for_day(self, option_id, appointment_date, appointee_id, criteria):
        ...
        slots = request.env['das.time.slot'].sudo().search([('day', '=', str(week_day))])
        ...
        for slot in slots:
            ...
            event_start = self.ld_to_utc(date_start + ' ' + functions.float_to_time(slot.slot), appointee_id).strftime("%Y-%m-%d %H:%M:%S")
            event_stop = self.ld_to_utc(date_start + ' ' + functions.float_to_time(slot.slot), appointee_id,
                                        duration=slot.slot).strftime("%Y-%m-%d %H:%M:%S")
    ...
    def get_days_with_free_slots(self, timeslot_id, year, month, criteria):
        ...
        for weekday, dates in start_datetimes.items():
            slots = request.env['das.time.slot'].sudo().search([('id', '=', timeslot_id),('day', '=', str(weekday))])
            slots = self.filter_slots(slots, criteria)
            ...
            for slot in slots:
                for d in dates:
                    ...
                    day_slots.append({
                        ...
                        'stop': self.ld_to_utc(d + ' ' + functions.float_to_time(slot.slot), appointee_id, duration=slot.slot).strftime("%Y-%m-%d %H:%M:%S")
                    })
    ...
    def month_bookable(self,timeslot_id,appointment_year, appointment_month, form_criteria, **kwargs):
