...
class ReportCashSalePopup(models.TransientModel):
    _name = 'report.cash.sale.popup'

    account_move_ids = fields.Many2many('account.move')

    @api.model
    def default_get(self, fields):
        res = super(ReportCashSalePopup, self).default_get(fields)
    ...
    def print_pdf(self, len):
        ...
        if account_move_ids.__len__() == 1:
            ...
            return self.env.ref('b20220713_print_invoice.report_cash_sales_id').report_action(invoice)
        for invoice_id in account_move_ids:
            invoice = self.env['account.move'].browse(invoice_id)
            pdf_report = self.env.ref('b20220713_print_invoice.report_cash_sales_id').render_qweb_pdf(invoice_id)
            data_record = base64.b64encode(pdf_report[0])
            ir_values = {
                'name': invoice._get_report_base_filename() + ".pdf",
                'type': 'binary',
                'datas': data_record,
                'store_fname': 'Cash Sale Report',
