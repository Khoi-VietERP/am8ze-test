...
class File(models.Model):
    ...
    file_history_ids = fields.One2many('muk_dms.file.history', 'file_id')
    image_text = fields.Text(string="Image Text", compute='get_image_text', store=True)
    ...
    def get_image_text(self):
        for rec in self:
            image_text = ''
            if 'image' in rec.mimetype:
                try:
                    content = rec.content_binary
                    ...
                    if content:
                        ...
                        for image in images:
                            ...
                            text = pytesseract.image_to_string(image)
                            image_text += text
            elif 'csv' in rec.mimetype or 'javascript' in rec.mimetype:
                content = rec.content_binary
                if content:
                    content = base64.b64decode(content)
                    image_text = content.decode("utf-8")
            elif 'msword' in rec.mimetype or rec.extension in ('doc','docx'):
                content = rec.content_binary
                content = base64.b64decode(content)
                f = io.BytesIO(content)
                image_text = docx2txt.process(f)
                image_text = image_text.replace('\n',' ')


            rec.image_text = image_text
        ...
        for record in self: 
            ...
            if not mimetype:
                try:
                    ...
                    mimetype = guess_mimetype(b'', default='application/octet-stream')
    ...
    def create(self, vals):
        ...
        self.env['muk_dms.directory.history'].create({
            'name': res.name,
            'description': 'Create File',
            'directory_id': res.directory.id,
        })
        return res
    ...
    def write(self, vals):
        ...
        if 'content' in vals:
            for rec in self:
                if rec.content:
                    self.env['muk_dms.file.history'].create({
                        'name' : rec.name,
                        'content' : rec.content,
                        'file_id' : rec.id,
                    })
        ...
        if 'directory' in vals:
            for rec in self:
                if vals.get('directory', False) != rec.directory.id:
                    if rec.directory:
                        self.env['muk_dms.directory.history'].create({
                            'name': rec.name,
                            'description': 'Delete File',
                            'directory_id': rec.directory.id,
                        })
