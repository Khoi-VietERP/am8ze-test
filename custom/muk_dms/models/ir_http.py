...
class IrHttp(models.AbstractModel):
    ...
    def binary_content(self, xmlid=None, model='ir.attachment', id=None, field='datas',
                       ...
                       access_token=None):
        res_status, res_headers, res_content = super(IrHttp, self).binary_content(xmlid=xmlid, model=model, id=id, field=field,
                                                                                  unique=unique, filename=filename, filename_field=filename_field, download=download,
                                                                                  mimetype=mimetype, default_mimetype=default_mimetype,access_token=access_token)
        if res_status == 200:
            env = self.env
            if model == "muk_dms.file" and field != 'content':
                obj = self._xmlid_to_obj(env, xmlid) if xmlid else env[model].browse(int(id))
