odoo.define('wk39717700c_modifier_report.ProductBatchStock', function (require) {
    'use strict';
    ...
    var _t = core._t;
    var exports = {};
    var ProductBatchStock = AbstractAction.extend({
        template: 'TempProductBatchStock',
        events: {
        ...
        plot_data: function (initial_render = true) {
            ...
            self._rpc({
                ...
                if (initial_render) {
                    self.$('.py-control-panel').html(QWeb.render('FilterProductBatchStock', {get_info: datas}));
                    self.$el.find('.location-multiple').select2({
                ...
                self.$('.py-data-container-orig').html(QWeb.render('TempProductBatchStockMain', {
                    get_info: datas,
                }));
        ...
        update_with_filter: function (event) {
            ...
            if ($(".product-multiple").select2('data').length === 1) {
                output.product_id = $(".product-multiple").select2('data')[0].id
            }
            if ($(".location-multiple").select2('data').length === 1) {
                output.location_id = $(".location-multiple").select2('data')[0].id
            }
            self._rpc({
                model: 'product.batch.stock',
                method: 'write',
        ...
        print_pdf: function (e) {
            ...
            self._rpc({
                ...
                action['context'] = {
                    'active_model': 'product.batch.stock',
    ...
    core.action_registry.add('product.batch.stock', ProductBatchStock);

    return {
        ProductBatchStock: ProductBatchStock,
    };
});
