odoo.define('h202_gst_reports_transactions.GstTransAccountMain', function (require) {
    'use strict';
    ...
    var _t = core._t;
    var exports = {};
    var GstTransAccountMain = AbstractAction.extend({
        template: 'GstTransAccountMain',
        events: {
        ...
        plot_data: function (initial_render = true) {
            ...
            self._rpc({
                ...
                if (initial_render) {
                    self.$('.py-control-panel').html(QWeb.render('FilterGstTransAccount', {get_info: datas,}));
                    self.$el.find('.account-multiple').select2({
                        placeholder: 'Select Account...',
                    });
                    self.$el.find('.account-multiple').css({"width": "280px"});
                }
                self.$('.py-data-container-orig').html(QWeb.render('GstTransAccountMainList', {
                    get_info: datas,
                }));
        ...
        print_pdf: function (e) {
            ...
            self._rpc({
                ...
                action['context'] = {
                    'active_model': 'gst.trans.account.code',
        ...
        update_with_filter: function (event) {
            ...
            if (output.start_date && output.end_date || output.account_ids) {
                self._rpc({
                    model: 'gst.trans.account.code',
                    method: 'write',
    ...
    core.action_registry.add('gst.trans.account', GstTransAccountMain);

    return {
        GstTransAccountMain: GstTransAccountMain,
    };
});
