...
class e_tax_wiz(models.TransientModel):
    ...
    def _sum_debit_account(self, account):
        ...
        self._cr.execute("""SELECT  SUM(account_move_line.debit)
                      FROM """ + tables + """
    ...
    def _sum_credit_account(self, account):
        ...
        if where_clause:
            where_clause = 'AND ' + where_clause
        self._cr.execute("""SELECT  SUM(account_move_line.credit)
                      FROM """ + tables + """
                      LEFT JOIN account_move am ON (am.id = account_move_line.move_id) \
                      WHERE (account_move_line.account_id = %s) \
                      AND (am.state IN %s) \
    ...
    def _sum_balance_account(self, account):
        ...
        if where_clause:
            where_clause = 'AND ' + where_clause
        self._cr.execute("""SELECT (sum(account_move_line.debit) - sum(account_move_line.credit)) as tot_balance \
                      FROM """ + tables + """
                      LEFT JOIN account_move am ON (am.id = account_move_line.move_id) \
                      WHERE (account_move_line.account_id = %s) \
                      AND (am.state IN %s) \
...
#             context = {}
        ...
        company_data = res_users_obj.browse(self._uid).company_id
        ...
        if date_from:
            cust_arg.append(('invoice_date', '>=', date_from))
            supp_arg.append(('invoice_date', '>=', date_from))

        if date_to:
            cust_arg.append(('invoice_date', '<=', date_to))
            supp_arg.append(('invoice_date', '<=', date_to))
        ...
        try:
            ...
            company_record = tools.ustr('CompInfoStart|||||||||||||') + \
                            ...
                            '|'.ljust(1) + \
                            tools.ustr(company_data and company_data.period_start and company_data.period_start.strftime('%d/%m/%Y') or '') + \
                            '|'.ljust(1) + \
                            tools.ustr(company_data and company_data.period_end and company_data.period_end.strftime('%d/%m/%Y') or '') + \
                            '|'.ljust(1) + \
                            tools.ustr(company_data and company_data.iaf_creation_date and company_data.iaf_creation_date.strftime('%d/%m/%Y') or '') + \
                            '|'.ljust(1) + \
            ...
            for supplier in supplier_invoice_ids:
                ...
                for line in supplier.invoice_line_ids:
                    ...
                    InvoiceDate = supplier and supplier.date_invoice and supplier.date_invoice.strftime('%d/%m/%Y') or ''
#                    InvoiceNo = supplier.supplier_invoice_number or ''
                    ...
                    if supplier.currency_id.id == supplier.company_id.currency_id.id:
                        ...
                        PurchaseValueSGD = cur_obj.with_context({'date': supplier.date_invoice}).compute(supplier.currency_id.id, supplier.company_id.currency_id.id, line.price_subtotal)
                        GSTValueSGD = 0.0
                        TaxCode = ''
                    ...
                    for tax in line.invoice_line_tax_ids:
                        ...
                        tax_name = ''
                        tax_data = tax.compute_all((line.price_unit * (1 - (line.discount or 0.0) / 100.0)), supplier.currency_id,line.quantity, line.product_id, supplier.partner_id)['taxes']
                        if tax_data:
                        ...
                        if supplier.currency_id.id == supplier.company_id.currency_id.id:
                            ...
                            GSTValueSGD = cur_obj.with_context({'date': supplier.date_invoice}).compute(supplier.currency_id.id, supplier.company_id.currency_id.id, tax_amt)
                            TaxCode += tax_name
                            GSTFCY = tax_amt
            ...
            for customer in customer_invoice_ids:
                ...
                for line in customer.invoice_line_ids:
                    ...
                    InvoiceDate = customer and customer.date_invoice and customer.date_invoice.strftime('%d/%m/%Y') or ''
                    ...
                    if customer.currency_id.id == customer.company_id.currency_id.id:
                        ...
                        SupplyValueSGD = cur_obj.with_context({'date': customer.date_invoice}).compute(customer.currency_id.id, customer.company_id.currency_id.id, line.price_subtotal)
                        GSTValueSGD = 0.0
                        TaxCode = ''
                    ...
                    for tax in line.invoice_line_tax_ids:
                        tax_amt = tax_amt_foreign = 0.0
                        tax_name = ''
                        tax_data = tax.compute_all((line.price_unit * (1 - (line.discount or 0.0) / 100.0)), customer.currency_id,line.quantity, line.product_id, customer.partner_id)['taxes']
                        if tax_data:
                            tax_amt = tax_data[0]['amount']
                            tax_rec = tax_obj.browse(tax_data[0].get('id'))
                            if tax_rec.tag_ids and tax_rec.tag_ids.ids:
                        ...
                        if customer.currency_id.id == customer.company_id.currency_id.id:
                            ...
                            GSTValueSGD = cur_obj.with_context({'date': customer.date_invoice}).compute(customer.currency_id.id, customer.company_id.currency_id.id, tax_amt)
                            TaxCode += tax_name
                            GSTFCY = tax_amt
                        tot_gst_sg += GSTValueSGD
            ...
            for report in child_data:
                if results[report.id].get('account'):
                    for acc,values in results[report.id]['account'].items():
                        ...
                        opening_balance = tools.ustr(date_from.strftime('%d/%m/%Y') or '') + \
                                          '|'.ljust(1) + \
                                          tools.ustr(acc.code) + \
                                          '|'.ljust(1) + \
                                          tools.ustr(acc.name) + \
                        ...
                        for ac in acc_data:
                            ...
                            if 'move_lines' in ac:
                                for move in ac['move_lines']:
                                    move_data = tools.ustr(move.get('ldate').strftime('%d/%m/%Y') or '') + \
                                                '|'.ljust(1) + \
                                                tools.ustr(acc.code) + \
                                                '|'.ljust(1) + \
                                                tools.ustr(acc.name) + \
                                                ...
                                                '|'.ljust(1) + \
                                                tools.ustr(float(move.get('debit')) or 0.0) + \
                                                '|'.ljust(1) + \
                                                tools.ustr(float(move.get('credit')) or 0.0) + \
                                                '|'.ljust(1) + \
                                                tools.ustr(move.get('progress') or 0.0) + \
                                                '|'.ljust(1) + \
                                                '\r\n'
