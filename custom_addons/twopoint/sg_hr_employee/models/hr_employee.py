...
class HrEmployee(models.Model):
    ...
    def _check_employee_birthday(self):
        """Constraints method to check valid birthday."""
        today = datetime.today()
        for rec in self:
            if rec.birthday:
                if rec.birthday > today.date():
                    raise ValidationError(
    ...
    def _get_month_date(self):
        for emp in self:
            ...
            birthday_day = 0
            if emp.birthday:
                birthday_month = emp.birthday.strftime('%m')
                birthday_day = emp.birthday.strftime('%d')
...
#    reason = fields.Text('Reason')
    ...
    birthday_month = fields.Char(compute='_get_month_date')
    birthday_day = fields.Char(compute='_get_month_date')
