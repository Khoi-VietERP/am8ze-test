...
class ins_account_financial_report(models.Model):
    ...
    def update_report_format(self):
        ...
        # 4-0100 Revenue
        account_ids = self.env['account.account'].search([('code', 'in', ['4-0100'])])
        self.create({
            'name': '4-0100 Revenue',
            'sequence': 0,
            'parent_id': pl_report_revenue_id.id,
        ...
        account_ids = self.env['account.account'].search([('code', 'in', ['5-0100','5-0200'])])
        self.create({
            'name': '4-0100 Revenue',
            'sequence': 0,
            'parent_id': pl_report_cost_of_sales_id.id,
            'display_detail': 'detail_with_hierarchy',
            'type': 'accounts',
            'range_selection': 'current_date_range',
            'sign': '1',
