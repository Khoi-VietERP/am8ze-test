...
#     _inherit = ['mail.thread', 'ir.needaction_mixin']
    ...
    def _write(self, vals):
        ...
        res = super(HelpdeskSupport, self)._write(vals)


        if res and vals.get('partner_id') and self.partner_id:
            """ added customer in followers """
            ...
            if not followers_obj.sudo().search([('res_id', '=', self.id), ('res_model', '=', 'helpdesk.support'),
                                                ('partner_id', '=', self.partner_id.id)]):
                followers_obj.sudo().create(reg)
        return res
    ...
    def create(self, vals):
        ...
        res = super(HelpdeskSupport, self.with_context(context, mail_create_nolog=True)).create(vals)

        if res and not res.partner_id and vals['email']:
            vals = {
                'name': vals['email'],
                'email': vals['email'],
            }
        ...
        if res and res.partner_id:
            ...
            if not followers_obj.sudo().search([('res_id', '=', res.id), ('res_model', '=', 'helpdesk.support'),
                                                ('partner_id', '=', res.partner_id.id)]):
                followers_obj.sudo().create(reg)

        """ send email for ticket open """
        template = self.env['mail.template'].sudo().search([('id','=',65)], limit=1)
        if template:
...
#                    line = self.env['account.move.line'].sudo().create(vals)
                    ...
                    invoice_line_id = invoice_line_obj.new(vals)
                    invoice_line_id._onchange_product_id()
                    inv_line = invoice_line_id._convert_to_write({name: invoice_line_id[name] for name in invoice_line_id._cache})
                    ...
                    line = self.env['account.move.line'].sudo().create(inv_line)
            invoice_id.sudo().compute_taxes()
        return True
...
#            else:
#                inv_obj = self.env['account.move'].sudo().search([('origin', '=', rec.name)])
...
#                ctx.update({'search_default_my_tasks' : 0})
...
#            res['domain'] = str([('id','in',rec.custom_project_task_ids.ids)])
