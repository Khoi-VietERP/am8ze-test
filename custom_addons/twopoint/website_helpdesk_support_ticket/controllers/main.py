...
class HelpdeskSupport(http.Controller):
    ...
    def index_user_invite(self, **kw):
        ...
        base_url = http.request.env['ir.config_parameter'].get_param('web.base.url',
                                                                     default='http://localhost:8069') + '/page/website_helpdesk_support_ticket.user_thanks'
        url = "%s?user_info=%s" % (base_url, user_info_id.id)
        reject_url = http.request.env['ir.config_parameter'].get_param('web.base.url',
                                                                       default='http://localhost:8069') + '/page/website_helpdesk_support_ticket.user_thanks_reject'
    ...
    @http.route(['/website_support_ticket/search_user_ticket'], type='http', auth="user", methods=['POST'],
                website=True)
