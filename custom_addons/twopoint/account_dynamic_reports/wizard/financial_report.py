...
class InsFinancialReport(models.TransientModel):
    ...
    def onchange_date_range(self):
        if self.date_range:
            ...
            if self.date_range == 'this_financial_year':
                ...
                if self.financial_year == 'last_feb_current_jan':
                    self.date_from = datetime(date.year - 1, 2, 1).strftime("%Y-%m-%d")
                    self.date_to = datetime(date.year, 1, 31).strftime("%Y-%m-%d")
    ...
    def get_account_lines(self, data):
        ...
        if data['enable_month_year_comp']:
            ...
            for cmp_per, cmp_dt in cmp_new_dates_list.items():
                ...
                new_comp_res = self.with_context(ctx_used_context)._compute_report_balance(child_reports)
            ...
            if res[report.id].get('account'):
                ...
                for account_id, value in res[report.id]['account'].items():
                    ...
                    if data['enable_month_year_comp']:
                        range_dict_2 = {}
                        for p in range(1, ctx_used_context.get('period')):
                            range_dict_2[p] = value[p]
                            # range_dict_2[p] = value[p] * int(report.sign)
                        vals['month_year_comp'] = range_dict_2
                    if flag:
    ...
    financial_year = fields.Selection(
        [('april_march', '1 April to 31 March'),
         ...
         ('last_feb_current_jan', '1 Feb last year to 31 Jan current year'),
         ],
    ...
    enable_month_year_comp = fields.Boolean(
        string='Enable Month Year Comparison',
        default=False)
    comp_period = fields.Integer(string='Period')
    comp_month_year_selection = fields.Selection([('month', 'Month'), ('year', 'Year')], string='Month/Year')
    cmp_end_date = fields.Date(string='Comp End Date')
