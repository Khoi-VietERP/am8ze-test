...
class HrPayslipExt(models.Model):
    ...
    def _get_ow_aw(self):
        for rec in self:
            rec.current_ow = current_ow = 0
            rec.current_aw = current_aw = 0
            ...
            payslip_id = rec.id
            contract_ids = [rec.contract_id.id] or \
                rec.get_contract(rec.employee_id, rec.date_from, rec.date_to)
            payslip_line_call = rec._get_payslip_lines(contract_ids, payslip_id)
            payslip_ids = rec.search([
                ('employee_id', '=', rec.employee_id.id),
                ('contract_id', '=', rec.contract_id.id),
                ('date_from', '>=', current_year_start_date),
                ('date_to', '<=', previous_month_last_date)])
            for pay in payslip_line_call:
                current_ow = pay.get('ow_total', 0.0)
                current_aw = pay.get('aw_total', 0.0)
                rec.current_aw = current_aw
                rec.current_ow = current_ow
